...
const ReviewLanding = () => {
  ...
  useEffect(() => {
    const loadCampaign = async () => {
      try {
        ...
        // Use fallback campaign if fetch failed
        const safeData = campaignData || {
          id: campaignId,
          name: 'Help Us Improve!',
          category: 'service',
          business_description: 'High-quality services tailored to customer needs',
          description: 'Share your feedback',
          google_review_url: '',
          location_id: null,
        };
        
        console.log('[ReviewLanding] Campaign safeData:', safeData);
        ...
        // Generate first review with business description
  ...
  const generateNewReview = async (campaignData: any) => {
    try {
      setGenerating(true);
      console.log('[ReviewLanding] Generating fresh review with business description');
      ...
      const businessDescription = campaignData?.business_description || campaignData?.description || 'Professional services';
      
      console.log('[ReviewLanding] Using description:', businessDescription);
      
      // Generate ONE fresh review using business description
      const reviews = await aiReviewService.generateReviews({
        businessName,
        businessCategory,
        businessDescription, // Pass the business description to AI
        numberOfReviews: 1,
        tone: 'professional',
        language: 'English',
      });
